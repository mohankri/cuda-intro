...
int main() {
    ...
    /*
     * Group the per-GPU collectives so NCCL schedules them as one operation.
     * Each rank contributes its local value, and the in-place AllReduce writes
     * the same sum back to every device buffer.
     */
